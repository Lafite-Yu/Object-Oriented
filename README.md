...
直接使用了`GUI`包中的地图读入和`BFS`算法，因此请不要对`GUI`包中的相关部分进行修改
...
定一个`FileWriter`类，该类用于处理所有的文件输出内容
...
4. 如果某车在被派单的时刻正处在两个点之间（即sleep 200ms时），将在醒来后再执行请求，但是执行请求前不会再随机移动一个点
5. 在某辆车被派单后，将立即转入接客状态，并在结束服务、将乘客送达目的地之前不再每20S停止1S，此外，在送达乘客、等候1S之后，将从零开始计算20S
6. 由于多线程执行的随机性和每条指令的执行都需要时间，边缘情况的执行情况不可预料，通过readme也无法准确规定，所以对于测试者来说请不要在例如接单检测窗口期开始和结束时的节点附近卡 bug。
7. `readme`未全面覆盖的问题，如果代码中有所体现，请以代码实际执行情况为准；如果没有考虑周全，敬请谅解
